...
export const SignupScreen: React.FC = () => {
  ...
  const showGenderPicker = () => {
    Alert.alert('Select your gender', '', [
      { text: 'Male', onPress: () => setGender('1') },
      { text: 'Female', onPress: () => setGender('2') },
      { text: 'Cancel', style: 'cancel' },
    ]);
  };

  const handleSignup = async () => {
    if (!firstName || !username || !email || !gender || !dob || !password || !confirmPassword) {
    ...
    // Convert dob to YYYY-MM-DD (API expects this format)
  ...
  const Input = ({ value, onChangeText, placeholder, focused, onFocus, onBlur, ...rest }: any) => (
    <RNTextInput
  ...
  return (
    ...
    >
        ...
        <View style={styles.content}>
          ...
          <View style={styles.form}>
            <FieldGroup label="First name" required>
              <Input value={firstName} onChangeText={setFirstName} placeholder="Enter first name" focused={focused.fn} onFocus={() => setFocused(f => ({ ...f, fn: true }))} onBlur={() => setFocused(f => ({ ...f, fn: false }))} autoCapitalize="words" />
            </FieldGroup>

            <FieldGroup label="Last name">
              <Input value={lastName} onChangeText={setLastName} placeholder="Enter last name" focused={focused.ln} onFocus={() => setFocused(f => ({ ...f, ln: true }))} onBlur={() => setFocused(f => ({ ...f, ln: false }))} autoCapitalize="words" />
            </FieldGroup>

            <FieldGroup label="Username" required>
              <Input value={username} onChangeText={setUsername} placeholder="Enter username" focused={focused.un} onFocus={() => setFocused(f => ({ ...f, un: true }))} onBlur={() => setFocused(f => ({ ...f, un: false }))} autoCapitalize="none" />
            </FieldGroup>

            <FieldGroup label="Email" required>
              <Input value={email} onChangeText={setEmail} placeholder="Enter email" keyboardType="email-address" focused={focused.em} onFocus={() => setFocused(f => ({ ...f, em: true }))} onBlur={() => setFocused(f => ({ ...f, em: false }))} autoCapitalize="none" />
            </FieldGroup>

            <FieldGroup label="Gender" required>
              <TouchableOpacity
                style={[styles.input, styles.pickerInput, focused.gen && styles.inputFocused]}
                onPress={showGenderPicker}
                onFocus={() => setFocused(f => ({ ...f, gen: true }))}
                onBlur={() => setFocused(f => ({ ...f, gen: false }))}
              >
                <Text style={[styles.pickerText, !gender && styles.placeholderText]}>
                  {gender === '1' ? 'Male' : gender === '2' ? 'Female' : 'Select your gender'}
                </Text>
                <Ionicons name="chevron-down" size={20} color={authColors.placeholderGray} />
              </TouchableOpacity>
            </FieldGroup>

            <FieldGroup label="Date of birth" required>
              <Input value={dob} onChangeText={setDob} placeholder="MM-DD-YYYY" focused={focused.dob} onFocus={() => setFocused(f => ({ ...f, dob: true }))} onBlur={() => setFocused(f => ({ ...f, dob: false }))} />
            </FieldGroup>

            <FieldGroup label="City">
              <Input value={city} onChangeText={setCity} placeholder="City" focused={focused.city} onFocus={() => setFocused(f => ({ ...f, city: true }))} onBlur={() => setFocused(f => ({ ...f, city: false }))} autoCapitalize="words" />
            </FieldGroup>

            <FieldGroup label="State">
              <Input value={state} onChangeText={setState} placeholder="State" focused={focused.state} onFocus={() => setFocused(f => ({ ...f, state: true }))} onBlur={() => setFocused(f => ({ ...f, state: false }))} autoCapitalize="words" />
            </FieldGroup>

            <FieldGroup label="Password" required>
              <View style={styles.passwordInputWrapper}>
                <RNTextInput
                  style={[styles.inputWithIcon, focused.pw && styles.inputFocused]}
                  value={password}
                  ...
                  autoCapitalize="none"
                />
                <TouchableOpacity style={styles.eyeButton} onPress={() => setPasswordVisible(!passwordVisible)} activeOpacity={0.7}>
                  <Ionicons name={passwordVisible ? 'eye-off-outline' : 'eye-outline'} size={22} color={authColors.primary} />
                </TouchableOpacity>
              </View>
            </FieldGroup>
            ...
            <FieldGroup label="Confirm password" required>
              <View style={styles.passwordInputWrapper}>
                <RNTextInput
                  style={[styles.inputWithIcon, focused.cpw && styles.inputFocused]}
                  value={confirmPassword}
                  onChangeText={setConfirmPassword}
                  onFocus={() => setFocused(f => ({ ...f, cpw: true }))}
                  onBlur={() => setFocused(f => ({ ...f, cpw: false }))}
                  placeholder="Confirm password"
                  placeholderTextColor={authColors.placeholderGray}
                  secureTextEntry={!confirmPasswordVisible}
                  autoCapitalize="none"
                />
                <TouchableOpacity style={styles.eyeButton} onPress={() => setConfirmPasswordVisible(!confirmPasswordVisible)} activeOpacity={0.7}>
                  <Ionicons name={confirmPasswordVisible ? 'eye-off-outline' : 'eye-outline'} size={22} color={authColors.primary} />
                </TouchableOpacity>
              </View>
            </FieldGroup>
            ...
            <View style={styles.bottomRow}>
              <View style={styles.signupRow}>
                ...
                <TouchableOpacity onPress={() => navigation.navigate(AUTH_ROUTES.LOGIN as any)} activeOpacity={0.8}>
      ...
      <View style={styles.bottomWave} pointerEvents="none">
        <Svg width={SCREEN_WIDTH} height={220} viewBox="0 0 916 465" preserveAspectRatio="none" style={styles.waveSvg}>
          <G opacity={0.5}>
...
const styles = StyleSheet.create({
  ...
  scrollContent: { flexGrow: 1, paddingHorizontal: 32, paddingBottom: 128 },
  ...
  pickerText: { fontSize: 14, color: authColors.textWhite },
  placeholderText: { color: authColors.placeholderGray },
  passwordInputWrapper: { position: 'relative', width: '100%' },
  inputWithIcon: { width: '100%', height: 45, backgroundColor: authColors.cardDark, borderRadius: 5, borderWidth: 1, borderColor: authColors.inputBorder, paddingVertical: 12, paddingLeft: 16, paddingRight: 48, fontSize: 14, color: authColors.textWhite },
  eyeButton: { position: 'absolute', right: 12, top: 0, bottom: 0, justifyContent: 'center' },
  bottomRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 20, marginBottom: 40, gap: 16, flexWrap: 'wrap' },
